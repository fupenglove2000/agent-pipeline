...
The initial implementation of `task_key` derivation (`normalize.derive_task_key`) computes `sha256(normalise(raw_input))` without a `source_id` component. `source_id` in the original formula above represents which batch or upstream system a task came from — a concept that belongs to the runner's notion of a run, which does not exist yet at this stage of the project. Requiring callers to inject a `source_id` field into `raw_input` before that concept is real would mean guessing at a schema for a feature that isn't built, which CLAUDE.md rules out.
This is a deliberate, temporary simplification, not an oversight: `source_id` is expected to be reintroduced into the key derivation once the runner defines what a batch/run actually is (Week 3 scope). Until then, two tasks from different sources with identical normalised content will share a `task_key` — which is the correct and only behaviour available without a defined source concept, not a bug to route around.
